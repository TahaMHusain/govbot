...
/// - If `govbot_output/repos/` exists with repos: just update existing repos (git pull)
/// - If `govbot_output/repos/` does not exist: clone repos based on govbot.yml config
pub fn run_pipeline(config_path: &Path) -> Result<()> {
    ...
    let repos_dir = cwd.join("govbot_output").join("repos");
