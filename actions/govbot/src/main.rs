...
enum Command {
    ...
    Clone {
        ...
        /// Directory containing repositories (default: $CWD/govbot_output/repos, or GOVBOT_DIR env var)
        #[arg(long = "govbot-dir")]
    ...
    Logs {
        ...
        /// Govbot directory (default: $CWD/govbot_output/repos, or GOVBOT_DIR env var)
        #[arg(long = "govbot-dir")]
        govbot_dir: Option<String>,        
    },
    ...
    /// The database file is saved in the base govbot directory (e.g., ./govbot_output/govbot.duckdb)
    Load {
...
fn get_govbot_dir(govbot_dir: Option<String>) -> anyhow::Result<PathBuf> {
    ...
    if let Some(govbot_dir) = govbot_dir {
        ...
        // Fall back to default: $CWD/govbot_output/repos
...
async fn run_load_command(cmd: Command) -> anyhow::Result<()> {
    ...
    // e.g., if repos_dir is ./govbot_output/repos, base_dir is ./govbot_output
...
async fn run_tag_command(cmd: Command) -> anyhow::Result<()> {
    ...
    // Model/tokenizer directory: prefer user-specified govbot-dir or env GOVBOT_DIR, else default govbot_output
    let model_dir: PathBuf = if let Some(ref dir) = govbot_dir {
        PathBuf::from(dir)
    } else if let Ok(dir) = std::env::var("GOVBOT_DIR") {
        PathBuf::from(dir)
    } else {
        current_dir.join("govbot_output")
    };
...
async fn run_build_command(cmd: Command) -> anyhow::Result<()> {
    ...
    if !base_govbot_dir.is_empty() && base_govbot_dir != "govbot_output" {
